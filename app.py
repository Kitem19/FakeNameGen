...
function copyToClipboard(element, text) {
    ...
    try {
        ...
        element.innerText = "✅"; // Icona di successo
        setTimeout(function() {
            element.innerText = "📋"; // Ripristina icona
...
def inbox_mailtm(info, auto_refresh_placeholder):
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("🔁 Controlla messaggi (Mail.tm)", key="check_tempmail"):
            st.session_state.auto_refresh = False
            with st.spinner("Recupero messaggi..."):
...
def display_profile_card(profile_data):
    ...
    def render_field(label, value):
        ...
        <div class="field-container">
            <span class="selectable-text-field">{value}</span>
            <span class="copy-icon" onclick="copyToClipboard(this, '{value}')" title="Copia {label}">📋</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    if email and "fallita" not in email:
        ...
        st.markdown(f"""
        <div class="field-container">
            <span class="selectable-text-field"><a href='mailto:{email}'>{email}</a></span>
